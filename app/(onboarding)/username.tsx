...
export default function UsernameStep() {
  ...
  const onFinish = async () => {
    ...
    if (!session?.user?.id) {
      await AsyncStorage.setItem('profile_draft', JSON.stringify({ ...draft, username: trimmed }));
    ...
    try {
      setSubmitting(true);
      // Vérifier l'unicité côté DB avant de continuer
      ...
      await AsyncStorage.mergeItem('profile_draft', JSON.stringify({ username: trimmed }));
      setSubmitting(false);
...
const styles = StyleSheet.create({
  ...
  card: { width: '100%', maxWidth: 480, gap: 8, backgroundColor: '#fff', borderRadius: 14, padding: 18, borderWidth: 1, borderColor: '#E5E7EB' },
